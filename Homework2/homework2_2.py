...
indif_0_d_index = []
for i in range(0, train_label.size):
    if train_label_indifference[i] == 0:
        prob = random.random()
        if prob > 0.75:
    ...
    elif train_label_indifference[i] == 1:
        prob = random.random()
        if prob > 0.5:
